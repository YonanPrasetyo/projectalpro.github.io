...
<div class="kanan">
        <h1>Mencari Data Karyawan</h1><hr>
        <p>anda saat ini akan mencari data dari database <span class="merah">
          <?php
        ...
        .txt</span></p><hr>
    <form method="post">
        <label for="dihapus">masukan NIK atau nama ingin anda lihat</label><br>
        <input type="text" name="dihapus" id="dihapus" required><br>
    ...
    if(isset($_POST['menghapus'])){
        ...
        /* mencari nik atau nama yang sesuai lalu menampikan datanya */
        $verif=false;
        for ($i=0; $i <= count($nikArr)-1; $i++) { 
            if ($nikdihapus==$nikArr[$i] || $nikdihapus==$namaArr[$i]) {
        ...
        /* ditampilkan jika tidak ditemukan data yang sesuai */
        if ($verif==false) {
            echo "<hr>data dengan NIK atau Nama ".$nikdihapus." tidak ditemukan";
        }
    }
    ?>
</div>
