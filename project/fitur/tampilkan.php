...
<div class="kanan">
    <h1>Data Karyawan</h1><hr>
        <p>dibawah ini adalah seluruh data dari database<span class="merah">
          <?php
    ...
    $file=fopen($databasepilihan.".txt","r");
        ...
        /* mengurutkan data berdasarkan nik nya */
        for ($i=0; $i <= count($nikArr)-1; $i++) { 
            $indexMin=count($nikArr)-1;
            for ($j=count($nikArr)-1; $j >= $i; $j--) { 
                if ($nikArr[$j]<$nikArr[$indexMin]) {
                    $indexMin=$j;
                }
            }
            $temp=$nikArr[$indexMin];
            $nikArr[$indexMin]=$nikArr[$i];
            $nikArr[$i]=$temp;

            $temp=$namaArr[$indexMin];
            $namaArr[$indexMin]=$namaArr[$i];
            $namaArr[$i]=$temp;

            $temp=$alamatArr[$indexMin];
            $alamatArr[$indexMin]=$alamatArr[$i];
            $alamatArr[$i]=$temp;

            $temp=$unitArr[$indexMin];
            $unitArr[$indexMin]=$unitArr[$i];
            $unitArr[$i]=$temp;

            $temp=$golonganArr[$indexMin];
            $golonganArr[$indexMin]=$golonganArr[$i];
            $golonganArr[$i]=$temp;
            
            $temp=$anakArr[$indexMin];
            $anakArr[$indexMin]=$anakArr[$i];
            $anakArr[$i]=$temp;

            $temp=$hariArr[$indexMin];
            $hariArr[$indexMin]=$hariArr[$i];
            $hariArr[$i]=$temp;

            $temp=$jam_kerjaArr[$indexMin];
            $jam_kerjaArr[$indexMin]=$jam_kerjaArr[$i];
            $jam_kerjaArr[$i]=$temp;
